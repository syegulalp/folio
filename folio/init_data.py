...
* `@start`: Newly-created wikis have their main page tagged with `@start` to indicate this is the starting page. You can reassign this tag to a new page to use that as the starting page. Wikis without `@start` defined will just default to the last edited page as the start page. Note that if you have multiple pages tagged as `@start`, the one with the *earliest lexographical ordering* for its title will display first. This feature will eventually become more refined.
